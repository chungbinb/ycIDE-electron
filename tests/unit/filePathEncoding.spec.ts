...
describe('文件命令中文路径 + 文本编码', () => {
  ...
  it.skipIf(!zigAvailable)('写到文件/读入文件：中文路径的字节集往返', async () => {
    ...
    // 到文本(字节集) 按易语言把字节当文本解（本 IDE 全程 UTF-8）——解回原文即往返无损
    expect(out).toContain('往返内容')
    // 调试输出(字节集) 仍是调试形态字节列表；「往返内容」UTF-8 = 12 字节，逐字节相同
    expect(out).toContain('字节集:12{229,190,128,232,191,148,229,134,133,229,174,185}')
  }, 120000)
})
